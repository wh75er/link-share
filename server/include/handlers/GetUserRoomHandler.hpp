#pragma once

#include <string>

#include "response.hpp"
#include "request.hpp"

template<class Model>
class GetUserRoomHandler: public BaseHandler<Model> {
public:
  explicit GetUserRoomHandler(BaseRequest request);
  ~GetUserRoomHandler() = default;

  void execute() override;

private:
  BaseRequest request_;
};

template<class Model>
GetUserRoomHandler<Model>::GetUserRoomHandler(BaseRequest request):
  request_(request)
{
}

template<class Model>
void GetUserRoomHandler<Model>::execute() {
  std::string error;

  std::shared_ptr<Model> model = this->get_model();
  std::shared_ptr<Response> response = this->get_response();

  if (!model) {
    error = "Model is not set!";
    response->error = error;
    return;
  }

  if(!response) {
    this->set_response(std::make_shared<Response>());
    response = this->get_response();
  }

  bool verified = false;
  try {
    verified = model->identify(request_.login, request_.token);
  }
  catch (const std::exception& e) {
    error = e.what();
    response->error = error;
    return;
  }

  if (!verified) {
    error = "Authorization error!";
    response->error = error;
    return;
  }

  std::string room_uuid;
  try {
    room_uuid = model->get_room_for_user(request_.login);
  }
  catch (const std::exception& e) {
    error = e.what();
    response->error = error;
    return;
  }

  response->error = error;
  response->uuid = room_uuid;
}
