...
template<class Json, class Model>
bool FormGetUserLinksRequest<Json, Model>::can_handle(int command) {
  return command == GET_USER_LINKS;
}

template<class Json, class Model>
std::shared_ptr<BaseHandler<Model>> FormGetUserLinksRequest<Json, Model>::spawn_handler(std::shared_ptr<Json> request_elements) {
  std::string login;
  if (!request_elements->get_value("login", login)) {
    throw ParserException(std::make_shared<ParseError>(ParseErrorCode::VALUE_NOT_FOUND), "login value is not presented in JSON!");
  }
  ...
  std::shared_ptr<Creator<SnapshotRequest, Model>> creator = std::make_unique<GetUserLinksHandlerCreator<SnapshotRequest, Model>>();
