...
public:
  ...
  virtual std::unique_ptr<BaseHandler> spawn_handler(std::vector<std::string>& request_elements) = 0;
};
...
public:
  ...
  std::unique_ptr<BaseHandler> spawn_handler(std::vector<std::string>& request_elements) override;
};
